...
uint64_t queue_push(uint64_t x) {
  if (write_head == ((write_tail - 1 + WRITE_QUEUE_SIZE) % WRITE_QUEUE_SIZE)) {
    return -1;
  }

  write_queue[write_head] = x;
  write_head = (write_head + 1) % WRITE_QUEUE_SIZE;
...
uint64_t queue_pop(uint64_t *x) {
  ...
  *x = write_queue[write_tail];
  write_tail = (write_tail + 1) % WRITE_QUEUE_SIZE;
...
void* shipper(void* ptr) {
  ...
  while(1) {
      ...
      if (!e) {
        printf("popped null pointer!? key=%li\n", key); 
      }
      else {
        //printf("WRITE key=%lu\n", key);
        CDABHeader cdh;
        ...
        events_written++;
        //int who = e->ptb_status;
        //uint64_t t = e->timetag;
        //fprintf(outfile, "%i, %lu\n", who, t);
      }
      //time_last_shipped = clock();
